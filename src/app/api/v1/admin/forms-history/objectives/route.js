...
// Objective lifecycle -> the badge vocabulary the Reports Hub renders.
const STATUS_MAP = { met: 'approved', in_progress: 'in_progress', not_started: 'draft' };
  ...
  try {
    ...
    const result = await withTenantClient(user.tenantId, user.staffId, async (client) => {
      ...
      const { rows } = await client.query(
        ...
        FROM care.objectives o
        LEFT JOIN care.goals g ON o.goal_id = g.id
