...
export async function GET(request) {
  ...
  try {
    ...
    // Advance Directive is now its own table (migration 0029): care.advance_directives.
    // The resident name lives in form_data; fall back to the linked pre-screening.
    const result = await withTenantClient(user.tenantId, user.staffId, async (client) => {
      const conditions = ['ad.tenant_id = $1', 'ad.deleted_at IS NULL'];
      const params = [user.tenantId];

      if (residentId) {
        params.push(residentId);
        conditions.push(`ad.resident_id = $${params.length}`);
      }
      if (startDate) {
        params.push(startDate);
        conditions.push(`COALESCE(ad.submitted_at, ad.created_at)::date >= $${params.length}`);
      }
      if (endDate) {
        params.push(endDate);
        conditions.push(`COALESCE(ad.submitted_at, ad.created_at)::date <= $${params.length}`);
      ...
      const { rows } = await client.query(
        `SELECT
          ad.id,
          'advance_directive' AS form_type,
          ad.resident_id,
          ad.form_data AS form_data,
          ps.client_full_name AS ps_encrypted_name,
          CONCAT(s.first_name, ' ', s.last_name) AS author,
          COALESCE(ad.submitted_at, ad.created_at) AS date_created,
          ad.status AS db_status,
          COUNT(*) OVER() AS total_count
        FROM care.advance_directives ad
        LEFT JOIN care.pre_admission_screenings ps ON ad.pre_screening_id = ps.id
        LEFT JOIN ref.staff s ON ad.created_by = s.id
        WHERE ${where}
        ORDER BY COALESCE(ad.submitted_at, ad.created_at) DESC
    ...
    return Response.json({
      data: result.map(row => {
        ...
        const residentName =
          fd.resident_name || fd.residentName || fd.name || fd.fullName ||
