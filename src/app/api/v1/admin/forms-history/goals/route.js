...
// Goal lifecycle -> the badge vocabulary the Reports Hub renders.
const STATUS_MAP = { achieved: 'approved', in_progress: 'in_progress', not_started: 'draft' };
  ...
  try {
    ...
    const result = await withTenantClient(user.tenantId, user.staffId, async (client) => {
      ...
      if (startDate) {
        ...
        conditions.push(`g.created_at::date >= $${params.length}`);
      }
      ...
      const { rows } = await client.query(
        `SELECT
          ...
          g.created_at AS date_created,
          CONCAT(s.first_name, ' ', s.last_name) AS author,
          g.status AS db_status,
          COUNT(*) OVER() AS total_count
        FROM care.goals g
        LEFT JOIN care.care_plans cp ON g.care_plan_id = cp.id
    ...
    return Response.json({
      data: result.map(row => {
        ...
        return {
          ...
          status,
          progressPercent: status === 'approved' ? 100 : status === 'draft' ? 25 : 75,
        };
      }),
      total,
      limit,
