  ...
  try {
    ...
    const result = await withTenantClient(user.tenantId, user.staffId, async (client) => {
      ...
      const { rows } = await client.query(
        `SELECT
          dpn.id,
          'daily_progress_notes' AS form_type,
          dpn.resident_id,
          r.first_name AS r_first_name,
          r.last_name  AS r_last_name,
          dpn.created_at AS date_created,
          CONCAT(s.first_name, ' ', s.last_name) AS author,
          dpn.review_status AS db_status,
          COUNT(*) OVER() AS total_count
        FROM care.daily_progress_notes dpn
        LEFT JOIN care.residents r ON dpn.resident_id = r.id
        LEFT JOIN ref.staff s ON dpn.staff_id = s.id
        WHERE ${where}
        ORDER BY dpn.created_at DESC
