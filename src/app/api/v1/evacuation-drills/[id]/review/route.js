 ...
 * Admin/manager approves or rejects an evacuation drill record (changes review_status).
 *
 * Path params:
 *   id (UUID) - evacuation drill ID
 *
 * Body:
 *   status (string, required) - one of: approved, rejected
 *   notes (string, optional) - review notes (e.g., improvements needed)
 *
 * Auth: admin, manager, or superadmin role only
 * Validation:
 *   - Returns 404 if evacuation drill not found in tenant
 *   - Returns 422 if status not approved or rejected
 *
 * Response: {
 *   id, status (review_status), reviewed_at,
 *   message: "Evacuation drill approved" or "Evacuation drill rejected"
 * }
 */
export async function PATCH(request, { params }) {
  try {
    ...
    if (!['admin', 'manager', 'superadmin'].includes(user.role)) {
      return Response.json({ error: 'Only admins and managers can review evacuation drills' }, { status: 403 });
    }
    ...
    const updated = await withTenantClient(user.tenantId, user.staffId, async (client) => {
      ...
      if (!existing.length) throw { status: 404, message: 'Evacuation drill not found' };
    ...
    return Response.json({
      ...
      message: `Evacuation drill ${status}`,
    });
  } catch (err) {
    if (err && err.status) {
