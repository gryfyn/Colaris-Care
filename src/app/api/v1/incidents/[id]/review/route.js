 ...
 * Admin/manager approves or rejects an incident report (changes review_status).
 *
 * Path params:
 *   id (UUID) - incident report ID
 *
 * Body:
 *   status (string, required) - one of: approved, rejected
 *   notes (string, optional) - review notes (e.g., follow-up action)
 *
 * Auth: admin, manager, or superadmin role only
 * Validation:
 *   - Returns 404 if incident not found in tenant
 *   - Returns 422 if status not approved or rejected
 *
 * Response: {
 *   id, status (review_status), reviewed_at,
 *   message: "Incident report approved" or "Incident report rejected"
 * }
 */
export async function PATCH(request, { params }) {
  try {
    ...
    if (!['admin', 'manager', 'superadmin'].includes(user.role)) {
      return Response.json({ error: 'Only admins and managers can review incidents' }, { status: 403 });
    }
    ...
    const updated = await withTenantClient(user.tenantId, user.staffId, async (client) => {
      ...
      if (!existing.length) throw { status: 404, message: 'Incident report not found' };
    ...
    return Response.json({
      ...
      message: `Incident report ${status}`,
    });
  } catch (err) {
    if (err && err.status) {
