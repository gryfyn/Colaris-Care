...
export async function GET(request) {
  return withApiContext(request, PERMISSIONS.RESIDENTS_READ, 'documents:read', async ({ client }) => {
    ...
    const { rows } = await client.query(
      `
        select id, resident_id, document_type, title, object_key, sha256, status, created_at
          from care.documents
         ${where}
    ...
    const { rows } = await client.query(
      `
        insert into care.documents(
          organization_id, facility_id, resident_id, document_type,
          title, object_key, sha256, status, created_by
        )
        values ($1, $2, $3, $4, $5, $6, $7, coalesce($8, 'active'), $9)
        returning id, resident_id, document_type, title, object_key, sha256, status, created_at
      `,
      [
        user.organizationId,
        user.facilityId,
