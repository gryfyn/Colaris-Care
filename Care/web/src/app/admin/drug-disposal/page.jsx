...
export default function AdminDrugDisposalPage() {
  ...
  useEffect(() => {
    let alive = true;
    apiData("/api/v1/drug-disposal").then((data) => alive && Array.isArray(data) && setRows(data.map(normalize))).catch(() => {});
    return () => { alive = false; };
  }, []);
  ...
  return (
    <div className="cx-wide">
      <PageHeader eyebrow="Medication safety" title="Drug disposal" lede="Review medication disposal records, quantities, witnesses, and status." />
